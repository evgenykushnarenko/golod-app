...
const WATER_KEY = 'water_count';

function initWaterTracker() {
  const addWaterBtn = document.getElementById('addWaterBtn');
  const waterCountEl = document.getElementById('waterCount');
  if (!addWaterBtn || !waterCountEl) return;

  let count = parseInt(localStorage.getItem(WATER_KEY) || '0', 10);
  const MAX = 8;

  function render() {
    waterCountEl.textContent = `${count}/${MAX}`;
    addWaterBtn.disabled = count >= MAX;
  }

  // Сбрасываем счётчик в полночь
  const today = new Date().toDateString();
  if (localStorage.getItem(WATER_KEY + '_date') !== today) {
    count = 0;
    localStorage.setItem(WATER_KEY, '0');
    localStorage.setItem(WATER_KEY + '_date', today);
  }
...
function init() {
  ...
  // Кнопки действий: в #actionButtonsContainer или после .hero-meta
  const actionContainer = document.getElementById('actionButtonsContainer');
  if (actionContainer) {
    actionContainer.appendChild(createActionButtons());
  } else {
    const heroMeta = document.querySelector('.hero-meta');
    if (heroMeta) heroMeta.after(createActionButtons());
  }
